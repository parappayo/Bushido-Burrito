package sim.actors 
{
	...
	public class PushBlock extends Prop
	{
		...
		{
			_img = new Image(Assets.ElementsTextures.getTexture("push"));
			addChild(_img);
						...
						else
						{
							...
							if (_framesPushed > 20) // roughly 0.5 sec
							{
								...
								if (game.getWalkmesh().isWalkable(_pushDestination.x, _pushDestination.y))
								{
									...
									SoundPlayer.play(Audio.boxMove, Audio.VOLUME_SFX_LOUD);
								}
							}
						}
					}
				...
				case STATE_MOVING:
					{
						...
						if (done)
						{
							...
							_img = new Image(Assets.ElementsTextures.getTexture("push_done"));
							addChild(_img);
						}
					}
					break;

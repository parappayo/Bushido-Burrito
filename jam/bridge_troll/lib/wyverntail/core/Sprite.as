...
package wyverntail.core
{
	...
	public class Sprite extends Component
	{
		...
		override public function start(prefabArgs :Object, spawnArgs :Object) :void
		{
			...
			if (prefabArgs.width) { img.width = prefabArgs.width; }
			if (prefabArgs.height) { img.height = prefabArgs.height; }
			if (prefabArgs.scaleX) { img.scaleX = prefabArgs.scaleX; }
			if (prefabArgs.scaleY) { img.scaleY = prefabArgs.scaleY; }
			img.smoothing = "none";
			addChild(img);

			if (_pos && !prefabArgs.pivotTopLeft)
			{
